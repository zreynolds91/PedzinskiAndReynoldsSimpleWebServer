...
public class DOSAttackWindow extends JFrame {
	...
	private void initComponents() {
		...
		this.lblThreadPool = new JLabel("Thread Pool");
		this.txtThreadPool = new JTextField("100");
		...
		this.panelServerConfig.add(this.lblThreadPool);
		this.panelServerConfig.add(this.txtThreadPool);
		

		// Compact server configuration panel
		SpringUtilities.makeCompactGrid(this.panelServerConfig, 3, 2, 5, 5, 5, 5);
		...
		contentPane.add(this.panelCommand, BorderLayout.SOUTH);	boolean execption = false;
	...
	private void addListeners() {
		...
		this.butStartServer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int threadPool = 100;
				try {
					threadPool = Integer.parseInt(DOSAttackWindow.this.txtThreadPool.getText());
				}
				catch(Exception ex) {
					JOptionPane.showMessageDialog(DOSAttackWindow.this, "Invalid Threadpool Value!", "Threadpool Value Problem", JOptionPane.ERROR_MESSAGE);
					return;
				}
				
				String server = DOSAttackWindow.this.txtServerName.getText();
				client = new DOSClient(DOSAttackWindow.this, server, port, threadPool);
	 ...
	 * @param e
	 */
	public synchronized void showSocketException(Exception e) {
		if(exceptionHandled)
			return;
